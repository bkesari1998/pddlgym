...
NUM_OBJECTS = 4
ROBOT, PROB, GOAL, EMPTY = range(NUM_OBJECTS)

EMPTY_CELL = np.zeros((200, 200, 3), np.uint8)
EMPTY_CELL[:] = [255, 255, 255]

PROB_CELL = np.zeros((200, 200, 3), np.uint8)
PROB_CELL[:] = [200, 200, 200]
...
def get_robot_location(obs):
    locs = []
    for lit in obs:
        if lit.predicate.name != 'at' or lit.predicate.is_negative:
            continue
...
def build_layout(obs, domain):
    ...
    for lit in obs:
        for v in lit.variables:
            if v.startswith('f'):
                ...
                max_r = max(max_r, r)
                max_c = max(max_c, c)
    layout = EMPTY * np.ones((max_r+1, max_c+1, 2), dtype=np.uint8)
    ...
    for r, c in get_robot_location(obs):
...
def render(obs, domain, close=False):
    ...
    return render_from_layout(layout, get_token_images, dpi=200)
